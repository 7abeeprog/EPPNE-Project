...
async def login(response: Response, request: Request, login_data: UserLogin, tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
    service = UserService(db, tenant.id)
    ...
    tokens = await service.generate_tokens(user, ip_address=ip, user_agent=ua)
...
@protected_router.put("/me", response_model=UserResponse)
async def update_me(user_in: UserUpdate, current_user: User = Depends(get_current_active_user), tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
    service = UserService(db, tenant.id)
    return await service.update_user(user_id=cast(int, current_user.id), data=user_in)

@router.post("/logout")
@rate_limit(max_requests=20, window_seconds=60)
async def logout(request: Request, response: Response, tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
...
async def refresh_token_endpoint(request: Request, response: Response, tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
    refresh_token = request.cookies.get("refresh_token")
...
@protected_router.post("/revoke-all")
@rate_limit(max_requests=5, window_seconds=300)
async def revoke_all_sessions(request: Request, current_user: User = Depends(get_current_active_user), tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
    service = UserService(db, tenant.id)
...
@protected_router.put("/me/password", status_code=status.HTTP_200_OK)
@rate_limit(max_requests=10, window_seconds=300)
async def change_password(request_data: ChangePasswordRequest, current_user: User = Depends(get_current_active_user), tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
    service = UserService(db, tenant.id)
...
@protected_router.delete("/me", status_code=status.HTTP_200_OK)
@rate_limit(max_requests=3, window_seconds=300)
async def soft_delete_account(request_data: DeleteAccountRequest, current_user: User = Depends(get_current_active_user), tenant: SimpleTenant = Depends(get_current_tenant), db: AsyncSession = Depends(get_db)):
    service = UserService(db, tenant.id)
