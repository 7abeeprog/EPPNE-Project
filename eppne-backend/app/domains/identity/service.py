...
class UserService:
    ...
    # ============================================================
    # 1. التسجيل (مع Idempotency Key و Atomicity)
    # ============================================================

    async def register(self, data: UserCreate, idempotency_key: Optional[str] = None) -> User:
        """إنشاء مستخدم جديد مع التحقق من Idempotency Key وإنشاء محفظة."""
        ...
        # 🔥 معاملة ذرية لإنشاء المستخدم والمحفظة
        async with self.db.begin_nested():
            ...
            await self.wallet_repo.create(cast(int, user.id))
    ...
    async def generate_tokens(self, user: User) -> Dict[str, str]:
        ...
        access_token = jwt_service.create_access_token(
            ...
            role=str(user.system_role),  # type: ignore
    ...
    async def refresh_tokens(self, refresh_token: str) -> Dict[str, str]:
        ...
        session_version = payload.get("sv")  # type: ignore
        ...
        await revoke_refresh_token(refresh_token)  # type: ignore
    ...
    async def revoke_all_sessions(self, user_id: int) -> int:
        ...
        # 🔥 معاملة ذرية لزيادة session_version وإبطال التوكنات
        async with self.db.begin_nested():
            await self.user_repo.update(user_id, session_version=User.session_version + 1)  # type: ignore
            revoked_count = await revoke_all_user_tokens(user_id)  # type: ignore
    ...
    async def get_current_user(self, user_id: int) -> User:
        """جلب المستخدم الحالي مع المحفظة (للاستخدام في الـ Router)."""
    ...
    async def change_password(self, user_id: int, old_password: str, new_password: str) -> bool:
        ...
        if not user or not verify_password(old_password, user.hashed_password):  # type: ignore
    ...
    async def link_wallet(self, user_id: int, wallet_address: str, signature_proof: str) -> bool:
        await self.user_repo.update(user_id, primary_wallet=wallet_address)  # type: ignore
        return True