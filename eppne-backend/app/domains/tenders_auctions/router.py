# app/domains/social/router.py (الإصدار النهائي المتكامل مع إضافة Tenders & Auctions)
...
from app.domains.tenders_auctions.schemas import (
    TenderCreate, TenderResponse, TenderBidCreate, TenderBidResponse,
    TenderBidEvaluate, LiveBidCreate, LiveBidResponse
)
...
async def create_tender(
    ...
    tender = await service.create_tender(current_user.id, tenant.id, data.model_dump())
    return tender
...
async def evaluate_bid(
    ...
    service = TendersAuctionsService(db)
    bid = await service.evaluate_bid_technically(
...
async def place_bid(
    ...
    bid = await service.place_bid(
        ...
        bid_amount=data.bid_amount_mrusdt,
        idempotency_key=idempotency_key
    )
    return bid
...
async def close_auction(
    ...
    service = TendersAuctionsService(db)
    result = await service.close_auction(auction_id, current_user.id, tenant.id)
    return result