...
class PrivacyRepository:
    ...
    async def get_privacy_settings(self, user_id: int) -> PrivacySetting:
        ...
        query: Any = select(PrivacySetting).where(PrivacySetting.user_id == user_id)
    ...
    async def get_erasure_request(self, request_id: int) -> Optional[DataErasureRequest]:
        """جلب طلب محو بيانات بواسطة المعرف."""
        query: Any = select(DataErasureRequest).where(DataErasureRequest.status == ErasureStatus.PENDING)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def get_pending_erasure_request(self, user_id: int, target_module: str) -> Optional[DataErasureRequest]:
    ...
    async def list_erasure_requests(
        ...
        """
        query: Any = select(DataErasureRequest).where(DataErasureRequest.user_id == user_id)
        if status:
        ...
        total = await self._estimate_row_count(DataErasureRequest.__tablename__, user_id, status)
    ...
    async def get_pending_erasure_requests_for_admin(
        ...
        total = await self._estimate_pending_count()
    ...
    async def update_erasure_request(self, request_id: int, **kwargs) -> DataErasureRequest:
        """تحديث طلب محو البيانات بالكامل."""
        request_obj = await self.get_erasure_request(request_id)
        if not request_obj:
    ...
    async def get_tombstones_by_user(
        ...
        total = await self._estimate_row_count(TombstoneRecord.__tablename__, user_id)
    ...
    async def update_tombstone_by_tx(self, tx_hash: str, field_name: str, status_value: Any) -> bool:
        """
        ...
        try:
            query: Any = (
                ...
                .where(TombstoneRecord.original_tx_hash == tx_hash)
    ...
    # ==========================================
    async def _estimate_row_count(self, table_name: str, user_id: int, status: Optional[ErasureStatus] = None) -> int:
        try:
            ...
            if estimated_total < 10000:
                count_query: Any = select(func.count()).select_from(
                    select(DataErasureRequest).where(DataErasureRequest.user_id == user_id)
                    .where(DataErasureRequest.status == status) if status else 
                    select(DataErasureRequest).where(DataErasureRequest.user_id == user_id)
                )
    ...
    async def _estimate_pending_count(self) -> int:
        try:
            ...
            if total < 10000:
                count_query: Any = select(func.count()).where(DataErasureRequest.status == ErasureStatus.PENDING)
                count_result = await self.db.execute(count_query)
                return count_result.scalar() or 0
    ...
    async def get_erasure_requests_with_tombstones(
        ...
        total = await self._estimate_row_count(DataErasureRequest.__tablename__, user_id)
