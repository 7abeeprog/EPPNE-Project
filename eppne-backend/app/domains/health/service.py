# app/domains/health/models.py (الإصدار النهائي المتكامل مع جميع التحسينات)
...
class FacilityCategory(str, enum.Enum):
    ...
    MARINE_BIOLOGY = "MARINE_BIOLOGY"  # للطحالب والكائنات البحرية
...
class BiometricSource(str, enum.Enum):
    ...
    DRONE_IMAGERY = "DRONE_IMAGERY"  # للكشف الزراعي
    SATELLITE = "SATELLITE"          # للرصد عن بعد
...
class HealthFacility(Base):
    ...
    supported_targets = Column(JSON, default=list)  # ["HUMAN", "ANIMAL", "PLANT"]
...
class MedicalProfile(Base):
    ...
    # ===== بيانات الكائن الحي =====
    species = Column(String(100), nullable=True)          # "Canis lupus", "Triticum aestivum"
    breed = Column(String(100), nullable=True)            # "Golden Retriever", "Wheat"
    plant_variety = Column(String(100), nullable=True)    # "Nile Wheat", "Soybean"
    scientific_name = Column(String(255), nullable=True)  # للطحالب والنباتات
