...
class EmploymentService:
    ...
    async def _check_saas_limits(self, tenant_id: int, feature: str = "hr_management"):
        ...
        if not features.get(feature, False):
            raise PermissionDeniedError("HR Management feature is not included in your current plan.")
    ...
    async def _calculate_ai_match_score(self, applicant_id: int, job: JobListing) -> Decimal:
        ...
        try:
            ...
            user_skills = user.skills if hasattr(user, 'skills') else []  # type: ignore
            prompt = f"""
            قم بتحليل التوافق بين المتقدم للوظيفة والوظيفة المعلنة.
            المهارات المطلوبة: {job.required_skills}  # type: ignore
            ...
            result = await self.ai_service.execute_agent_action(
                agent_id=1,
                tenant_id=job.tenant_id,  # type: ignore
    ...
    async def apply_for_job(
        ...
        if not job or not job.is_active:  # type: ignore
        ...
        if job.required_certificate_ids:  # type: ignore
            certificates = await self.academy.get_user_certificates(applicant_id)  # type: ignore
            cert_course_ids = [c.course_id for c in certificates]  # type: ignore
            missing = set(job.required_certificate_ids) - set(cert_course_ids)  # type: ignore
            if missing:
    ...
    async def get_job_applications(self, employer_id: int, job_id: int, skip: int = 0, limit: int = 50) -> List[JobApplication]:
        job = await self.repo.get_job_listing_by_employer(job_id, employer_id)
        if not job:
    ...
    async def review_application(
        ...
        job = await self.repo.get_job_listing(app.job_id)  # type: ignore
        if not job or job.employer_id != employer_id:  # type: ignore
    ...
    async def create_contract(
        ...
        if application.status != "APPROVED":  # type: ignore
            raise PermissionDeniedError("لا يمكن إنشاء عقد إلا لطلب مقبول")
        job = await self.repo.get_job_listing(application.job_id)  # type: ignore
        if job.employer_id != employer_id:  # type: ignore
        ...
        async with self.db.begin_nested():
            contract = await self.repo.create_contract(
                ...
                employee_id=application.applicant_id,  # type: ignore
                employer_id=employer_id,
                job_title=bleach.clean(data.get("job_title", job.title), tags=[], strip=True),  # type: ignore
        ...
        await audit_log(
            ...
            resource_id=contract.id,  # type: ignore
    ...
    async def sign_contract(
        ...
        if is_employee and contract.employee_id != user_id:  # type: ignore
            raise PermissionDeniedError("ليس لديك صلاحية توقيع هذا العقد")
        if not is_employee and contract.employer_id != user_id:  # type: ignore
            raise PermissionDeniedError("ليس لديك صلاحية توقيع هذا العقد")
        if is_employee:
        ...
        if contract.signature_tx_hash and contract.employer_signature_tx:  # type: ignore
    ...
    async def check_in(
        ...
        contract = await self.repo.get_contract(contract_id)
        if not contract or contract.employee_id != user_id:  # type: ignore
            raise PermissionDeniedError("هذا العقد لا يخصك")
        if contract.status not in [EmploymentStatus.ACTIVE, EmploymentStatus.PROBATION]:  # type: ignore
            raise PermissionDeniedError("العقد غير نشط")
        today_record = await self.repo.get_today_attendance(contract_id)
        if today_record and today_record.check_in:  # type: ignore
            raise PermissionDeniedError("تم تسجيل الحضور مسبقاً اليوم")
        if not (-90 <= latitude <= 90 and -180 <= longitude <= 180):
            raise ValueError("إحداثيات غير صالحة")
        if today_record:
            today_record.check_in = datetime.utcnow()  # type: ignore
            today_record.check_in_location = {"lat": latitude, "lng": longitude}  # type: ignore
            today_record.status = AttendanceStatus.PRESENT  # type: ignore
    ...
    async def check_out(
        ...
        if not contract or contract.employee_id != user_id:  # type: ignore
            raise PermissionDeniedError("هذا العقد لا يخصك")
        today_record = await self.repo.get_today_attendance(contract_id)
        if not today_record or not today_record.check_in:  # type: ignore
            raise PermissionDeniedError("لم يتم تسجيل حضور اليوم")
        if today_record.check_out:  # type: ignore
        ...
        return await self.repo.update_attendance_check_out(cast(int, today_record.id), datetime.utcnow(), location)

    async def get_my_attendance(self, user_id: int, contract_id: int, skip: int = 0, limit: int = 30) -> List[AttendanceRecord]:
        contract = await self.repo.get_contract(contract_id)
        if not contract or contract.employee_id != user_id:  # type: ignore
            raise PermissionDeniedError("لا تملك صلاحية الاطلاع على هذا السجل")
        year = datetime.utcnow().year
        month = datetime.utcnow().month
        records = await self.repo.get_attendance_for_month(contract_id, year, month)
    ...
    async def request_leave(
        ...
        if leave_type == LeaveType.ANNUAL:
            ...
            if used_days + requested_days > contract.annual_leave_days:  # type: ignore
    ...
    async def approve_leave(self, employer_id: int, leave_id: int, approve: bool) -> LeaveRequest:
        ...
        contract = await self.repo.get_contract(leave.contract_id)  # type: ignore
        if contract.employer_id != employer_id:  # type: ignore
    ...
    async def generate_payroll(
        ...
        if not contract or contract.employer_id != employer_id:  # type: ignore
        ...
        payroll = await self.repo.create_payroll(
            ...
            contract_id=contract_id,
            month=month,
            base_salary=contract.base_salary,  # type: ignore
            bonuses=Decimal(0),
    ...
    async def approve_payroll(self, payroll_id: int, employer_id: int) -> PayrollRecord:
        ...
        contract = await self.repo.get_contract(payroll.contract_id)  # type: ignore
        if contract.employer_id != employer_id:  # type: ignore
    ...
    async def pay_payroll(
        ...
        if idempotency_key:
            ...
            if cached:
                return cached
        payroll = await self.repo.get_payroll(payroll_id)
        if not payroll or payroll.status != PayrollStatus.APPROVED:  # type: ignore
            raise PermissionDeniedError("كشف الراتب غير معتمد أو غير موجود")
        contract = await self.repo.get_contract_with_tenant(payroll.contract_id, tenant_id)  # type: ignore
        if contract.employer_id != employer_id:  # type: ignore
    ...
    async def get_my_payrolls(self, user_id: int, skip: int = 0, limit: int = 12) -> List[PayrollRecord]:
        ...
        return await self.repo.list_payrolls_for_contract(contract.id, contract.tenant_id, skip, limit)  # type: ignore