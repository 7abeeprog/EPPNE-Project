// app/(dashboard)/invitations/tickets/page.tsx
'use client';

import { useState } from 'react';
import { useTickets } from '@/hooks/invitations/useTickets';
import TicketCard from '@/components/invitations/TicketCard';
import TicketStatusBadge from '@/components/invitations/TicketStatusBadge';
import { Loader2, Ticket, Search, Filter, Plus } from 'lucide-react';
...
export default function TicketsPage() {
  ...
  const { data: tickets, isLoading } = useTickets({ ...(statusFilter && { status: statusFilter }) });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground/90">🎫 تذاكر الدعم</h1>
        <Link
          href="/invitations/tickets/create"
      ...
      {filtered?.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground/60">
          ...
          <p className="text-lg">لا توجد تذاكر دعم</p>
          <p className="text-sm">كل تذاكر الدعم ستظهر هنا</p>
        </div>
      ) : (
