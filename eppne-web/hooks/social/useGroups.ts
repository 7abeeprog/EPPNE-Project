...
import { getGroups, leaveGroup, getGroupMembers } from '@/services/social';
...
export const useGroup = (id: number) => {
  return useQuery({
    ...
    queryFn: () => SocialService.getGroup(id),
    enabled: !!id,
    staleTime: 2 * 60 * 1000,
  });
};
...
export const useCreateGroup = () => {
  ...
  return useMutation({
    mutationFn: (data: Parameters<typeof SocialService.createGroup>[0]) => SocialService.createGroup(data),
    onSuccess: () => {
...
export const useJoinGroup = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (groupId: number) => SocialService.joinGroup(groupId),
    onSuccess: (_, groupId) => {
