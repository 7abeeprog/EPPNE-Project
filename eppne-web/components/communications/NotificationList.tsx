...
import { getNotifications } from '@/services/communications.service';
...
export default function NotificationList({ onItemClick }: { onItemClick?: () => void }) {
  ...
  const { mutate: markRead } = useMutation({ mutationFn: CommunicationsService.markNotificationRead, onSuccess: (_, id) => { queryClient.invalidateQueries({ queryKey: ['notifications'] }); decrementUnread(); } });
