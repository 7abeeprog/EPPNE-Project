...
آخر إغلاق رسمي: **Backlog #11b (`invoicing-savepoint-conflict`) [2026-08-18]** — 8 دوال عبر 4 دومينات (`tourism_sports` 3، `realestate` 2، `insurance` 2، `zamakana` 1). راجع الجدول تحت وتفصيل كامل في `.claude/reports/realestate-insurance-savepoint-fix-session-log.md`.

**ملخص الإغلاق:** الحل (نقل `invoicing.create_invoice()` من جوه `begin_nested()` إلى بعد `commit()` الخارجي، ملفوف بـ`try/except`) طُبِّق على الثمانية دوال. **تحقق حي كامل بـSELECT/Redis مستقل** لـ4 دوال (`purchase_event_ticket`, `rent_unit`, `zamakana.pledge_time`، وجزء من `insurance.subscribe`) — صفر باج جديد اتكشف فيهم. **مراجعة ديف/كود فقط (بلا تحقق حي)** للأربعة الباقية (`book_program`, `place_transfer_bid`, `buy_fractional_ownership`, `review_claim`) — محجوبين ببجات مسبقة **منفصلة تمامًا عن #11b**، اتكشفت أثناء الجلسة ووُثِّقت كـ3 بنود Backlog جديدة مستقلة (راجع الجدول تحت: `tourism-place-transfer-bid-player-id-user-id-conflict`, `finance-transfer-returns-transaction-object-not-tx-hash-string`, `eventbus-redis-wrapper-missing-publish`).

**✅ القيد على Backlog #9 اتشال [2026-08-18]:** كان مفروضًا بسبب #11b — دلوقتي #11b مُغلَقة، مسموح فتح جلسة إصلاح لـ#9. **تنبيه صريح (مش اعتماد صدفة جديد على #11b نفسها، لكن يستأهل الانتباه):** لو #9 اتصلحت، `realestate.buy_fractional_ownership`/`insurance.review_claim` هيوصلوا فعليًا لباج **مختلف تمامًا** (`finance-transfer-returns-transaction-object-not-tx-hash-string`) بدل ما ينجحوا بالكامل — يستأهل تنسيق مع جلسة #9 القادمة.
...
| 9 | `saas-control-service-missing-methods` (`get_active_subscription`) | 🔴 **مفتوح — القيد اتشال [2026-08-18]، #11b مُغلَقة.** تنبيه: لو اتصلحت، `realestate.buy_fractional_ownership`/`insurance.review_claim` هيوصلوا لباج مختلف (`finance-transfer-returns-transaction-object-not-tx-hash-string`) بدل النجاح الكامل. | `.claude/reports/saas-control-service-missing-methods-session-log.md`، `.claude/reports/realestate-insurance-savepoint-fix-session-log.md` |
